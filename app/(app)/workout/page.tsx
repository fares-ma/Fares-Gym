...
export default async function WorkoutPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="comic-card p-5 sm:p-6 border border-[#2B252E] flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="space-y-1 text-right w-full sm:w-auto">
          <div className="flex items-center gap-2">
            <span className="text-xs font-black tracking-widest text-[#D6AA63] uppercase font-mono">
              WORKOUT HUB
            </span>
            <span className="comic-badge text-[10px]">4-DAY CYCLE</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-black text-[#F2EADF] tracking-tight flex items-center gap-2.5">
            <Dumbbell className="w-6 h-6 text-[#7C1D38]" />
            <span>{ar.workout.title}</span>
          </h1>
          <p className="text-xs sm:text-sm text-[#9D969D]">
            {ar.workout.subtitle}
          </p>
          ...
          <div className="pt-2">
            <Link
              ...
              className="inline-flex items-center gap-2 text-xs font-bold text-[#D6AA63] hover:text-[#F2EADF] transition-colors"
            >
      ...
      {activeData && (
        <div className="comic-card-accent p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-[#2B101D] to-[#18151B]">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-xl bg-[#7C1D38]/30 text-[#E0537A] flex items-center justify-center shrink-0 mt-0.5 animate-pulse">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-black text-[#F2EADF]">
            {ar.workout.tabs.programs}
          </h2>
          <span className="text-xs text-[#9D969D] font-mono">
